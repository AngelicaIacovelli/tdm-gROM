...
class MGNTrainer:
    def __init__(self, logger, cfg, dist):
        ...
        dt = 0.8/ cfg.LNODEs_architecture.N_timesteps

        # instantiate the model
        self.model = LNODECell(cfg, dt)
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=cfg.LNODEs_architecture.lr)
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer,
            T_max=cfg.LNODEs_architecture.epochs,
            eta_min=cfg.LNODEs_architecture.lr * cfg.LNODEs_architecture.lr_decay,
        )
        self.scaler = GradScaler()

        # load checkpoint
        self.epoch_init = load_checkpoint(
            os.path.join(cfg.LNODEs_architecture.checkpoints_ckpt_path, cfg.LNODEs_architecture.checkpoints_ckpt_name),
            models=self.model,
...
def do_training(cfg, dist):
    ...
    for epoch in range(trainer.epoch_init, cfg.LNODEs_architecture.epochs):
        # Loop over batches
        ...
        if cfg.training.output_interval != -1:
            if (epoch % cfg.training.output_interval) == 0 or epoch == 0 or epoch == (cfg.LNODEs_architecture.epochs-1): 
                # save checkpoint
                save_checkpoint(
                    os.path.join(cfg.LNODEs_architecture.checkpoints_ckpt_path, cfg.LNODEs_architecture.checkpoints_ckpt_name),
        ...
        if cfg.training.output_interval != -1:
            with open(cfg.LNODEs_architecture.checkpoints_ckpt_path + "/parameters.json", "w") as outf:
