...
class GLSTMCell(Module):
    ...
    def encode_edges(self, edges):
        ...
        """
        enc_features = edges.data["efeatures"]
        return {"proc_edge": enc_features}
    ...
    def compute_Uih(self, edges, g): #togli g 
        weight_matrix_U = self.U_i.weight
        # print("This is U_i: ", weight_matrix_U)
    ...
    def compute_i(self, nodes):
        x = nodes.data["proc_node"].clone()  # dictionary (key: 'proc_node', value: processed features)
    ...
    def compute_o(self, nodes):
        x = nodes.data["proc_node"].clone() 
        Uoh_sum = nodes.data["Uoh_sum"].clone() 
        Wx = self.W_o(x)
        o = Wx + Uoh_sum
        o = th.sigmoid(o)
        # print("o: ", o)
    ...
    def compute_u(self, nodes):
        x = nodes.data["proc_node"].clone() 
        Uuh_sum = nodes.data["Uuh_sum"].clone() 
        Wx = self.W_u(x)
        u = Wx + Uuh_sum
        u = th.tanh(u)
        # print("u: ", u)
    ...
    def compute_fc(self, edges,g,cfg):
        f = edges.data["f"].clone() 
        #c = edges.src["c"]
        c = th.zeros(
            (g.number_of_nodes(), cfg.architecture.hidden_dim),
            dtype=th.float32,
        )
        fc = f * (th.sigmoid(c))
        return {"fc": fc}
    ...
    def compute_c(self, nodes):
        i = nodes.data["i"].clone() 
        u = nodes.data["u"].clone() 
        fc_sum = nodes.data["fc_sum"].clone()   # Assuming 'fc_sum' already exists or is initialized.
        c = i * u
        c = fc_sum + c
        # print("c: ", c)
        return {"c": c}
    ...
    def compute_h(self, nodes):
        o = nodes.data["o"].clone() 
        c = nodes.data["c"].clone() 
        c = th.tanh(c)
        h = o * c
        # print("h: ", h)
        return {"h": h}
    ...
    def restart(self, g):
        g.ndata["h"] = 0 * g.ndata["h"]
        g.ndata["c"] = 0 * g.ndata["c"]
    ...
    def forward(self, g):
        ...
        for i in range(self.autoloop_iterations):
            ...
            # Vector i
            g.apply_edges(self.compute_Uih(g=g))
            g.update_all(fn.copy_e("Uih", "m"), fn.sum("m", "Uih_sum"))
            g.apply_nodes(self.compute_i)
            ...
            # Vector o
            g.apply_edges(self.compute_Uoh(g))
            g.update_all(fn.copy_e("Uoh", "m"), fn.sum("m", "Uoh_sum"))
            g.apply_nodes(self.compute_o)

            # Vector u
            g.apply_edges(self.compute_Uuh(g))
            g.update_all(fn.copy_e("Uuh", "m"), fn.sum("m", "Uuh_sum"))
            g.apply_nodes(self.compute_u)

            # Vector c
            g.apply_edges(self.compute_fc(g))
            g.update_all(fn.copy_e("fc", "m"), fn.sum("m", "fc_sum"))
            g.apply_nodes(self.compute_c)
